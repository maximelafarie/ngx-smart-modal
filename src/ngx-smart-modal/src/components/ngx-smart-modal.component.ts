...
@Component({
  ...
  template: `
    <div *ngIf="overlayVisible" class="overlay {{ openedClass ? 'nsm-overlay-open' : 'nsm-overlay-close'}}" [style.z-index]="visible ? layerPosition-1 : -1"
         [ngClass]="{'transparent':!backdrop}"
         (click)="dismiss($event)">
    </div>
    <div [style.z-index]="visible ? layerPosition : -1" class="nsm-dialog-animation-fade nsm-dialog {{ openedClass ? 'nsm-dialog-open' : 'nsm-dialog-close'}}" [ngClass]="customClass">
...
export class NgxSmartModalComponent implements OnInit, OnDestroy {
  ...
  private openedClass: boolean = false;
