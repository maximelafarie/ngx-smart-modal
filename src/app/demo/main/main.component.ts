...
import { zip, Subscription, fromEvent, from } from 'rxjs';
...
export class MainComponent implements AfterViewInit, OnDestroy {
  ...
  // Create a Subscriber to unsubscribe subs to observables
  ...
  ngAfterViewInit() {
    ...
    this.ngxSmartModalService
      .getModal('classicModal')
      .onEscape.subscribe((modal: NgxSmartModalComponent) => {
        console.log('You just escaped the classicModal!', modal);
      });

    this.ngxSmartModalService
      .getModal('classicModal')
      .onAnyCloseEventFinished.subscribe((modal: NgxSmartModalComponent) => {
        console.log('All close events are ended!', modal);
      });

    const openEvents = from(this.ngxSmartModalService.getModalStack()).pipe(mergeMap(modal => modal.modal.onOpen));

    // Listen for all modal open events
    this.subs.add(
      openEvents.subscribe((modal: NgxSmartModalComponent) => {
        console.log('event');
        this.googleAnalyticsService.eventEmitter('modal_open', 'demo', 'open', 'click', modal.ariaLabel || modal.identifier);
      })
    );
  ...
  goNewDynamic() {
    ...
    this.ngxSmartModalService
      .create('dynamicModal1', "Hello, I'm a simple text !")
      .open();

    this.ngxSmartModalService
      .create('dynamicModal2', FakeComponent, opts)
      .open();
